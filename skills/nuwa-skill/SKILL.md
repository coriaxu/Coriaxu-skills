...
#### Research Tooling (Enhanced with web-access + agent-reach)

Each subagent MUST use `web-access` skill as its primary research engine (instead of basic WebSearch/WebFetch). This enables dynamic page rendering, CDP browser mode, and deeper content extraction via Jina Reader.

For **platform-specific content**, each subagent should use `agent-reach` skill:
- **Twitter/X**: tweets, threads, X Articles
- **YouTube**: interview transcripts and talk subtitles
- **Bilibili (B站)**: Chinese video content and danmaku
- **小红书**: posts and comments
- **微信公众号**: articles from the person's own official account (treated as PRIMARY source)
- **Reddit / GitHub**: discussions, repos, Q&A threads
- **Podcast platforms**: episode descriptions and show notes

**Invocation**: each subagent should call `Skill("web-access")` or `Skill("agent-reach")` at the start of its research to load instructions, then follow those instructions for all web operations.
...
#### Source Blocklist & Quality Filter

For Chinese figures:
- **Exclude**: Zhihu (知乎) answers, Baidu Baike/Zhidao, random WeChat reposts
- **Allow as PRIMARY source**: The person's own 微信公众号 official account articles (use agent-reach or web-access to extract)
- **Allow as authoritative secondary**: 36Kr, GeekPark, LatePost, Caixin, Yicai, Huxiu, SSPAI, 少数派, 晚点LatePost
